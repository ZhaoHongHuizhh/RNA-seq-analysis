...
### 获取样本信息 -----------
setwd("C:/Users/ZHH/Desktop/G12D")
...
count_file <- list.files("C:/Users/ZHH/Desktop/lung/lung",pattern = '*.tsv',recursive = TRUE)
#在count_file中分割出文件名
...
for (i in 1:length(count_file)){
  path = paste0('C:/Users/ZHH/Desktop/lung/lung//',count_file[i])
...
exprset <- read.csv('COUNT_matrix.csv')
metadata <- data.frame(TCGA_id =colnames(exprset)[-1])
table(substring(metadata$TCGA_id,14,15))
### 设置Gene Symbol为列名的矩阵（前面得到的是Ensembl ID）------------------------------------------
path = paste0('C:/Users/ZHH/Desktop/lung/lung//',count_file[1])
