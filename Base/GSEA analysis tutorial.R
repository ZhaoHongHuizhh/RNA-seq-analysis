...
### Set the path
setwd("C:/Users/ZHH/Desktop/Bulk rna-seq")

### install the R package for the analysis 
...
diff <- read.csv('diff.csv',header = T)
...
write.csv(GSEA.KEGG.result ,file = "GSEA_KEGG.csv")

### GSEA Visualization of results
mygene <- c("KRAS","SPP1","MYC","TP53","CDH1")
...
# add segment line
dotplotGsea(data = KEGG.res,topn = 10,
            order.by = 'NES',
            add.seg = T)
...
### Save the data
write.csv(KEGG_ges_result,file = "GSEA_KEGG.csv")
...
df <- diff[order(diff$log2FoldChange,decreasing = T),]
...
fwrite(GSEA_df, file="GSEA_df.csv")

### The GSEA analysis by R was end -----------------
